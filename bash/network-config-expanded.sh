...
# Identify the network number for this interface and its name if it has on
interfacelist=$(lshw -class network | awk '/logical name:/{print $3}' | wc -l)
#creating a loop for running the gathered information
for((K=1;K<=$interfacelist;K+=1));
do
...
#the gathered information is used for the loopback
...
# to print the infomation we use echo
